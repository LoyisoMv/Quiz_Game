...
mediumlevel::mediumlevel(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::mediumlevel)
{
    ui->setupUi(this);
}
...
mediumlevel::mediumlevel(QString str, QWidget *parent)
    :QDialog(parent), ui(new Ui::mediumlevel){
    ...
    connect(getNextBtn(),&QPushButton::clicked,this,[this](){
        ...
        if(curr > 8){
            //Global::instance().addToT(td);
            time->stop();
            this->close();
            if(levelScore > 4){
                //setfeedbackUIWidgets();
                Global::instance().addMediumScore(levelScore);
                int completeTime;
                completeTime = 180 - td;
                Global::instance().addMediumTime(completeTime);
                setfeedbackUIWidgets();
            }
            else{
                setFailureUI();
...
void mediumlevel::displayCurrentQuestion(){
...
void mediumlevel::progress(QPushButton* btn){
...
void mediumlevel::controlOptionButton(QPushButton* btn1,QPushButton* btn2,QPushButton* btn3){
...
void mediumlevel::progress(int currQuestionNum,QPushButton* currQuestionBtn,QPushButton *btn){
    if(currQuestionNum == currQuestionBtn->text().toInt())
        currQuestionBtn->setStyleSheet(btn->styleSheet());
}

void mediumlevel::setProgressButtonStyle(QPushButton* currQuestionBtn){
...
void mediumlevel::restartTimer(){
    td = 180;
}

void mediumlevel::setfeedbackUIWidgets()
{
    ...
    feedBackUi->getCompletedLevelLabel()->setText("Medium Level");
    int completeTime = 0;
...
void mediumlevel::updateScore()
{
    levelScore++;
}

void mediumlevel::setFailureUI()
{
    feedBackUi->showMaximized();
    ...
    feedBackUi->getLevelTimeLabel()->setText(QString("%1: %2").arg(minutes,2, 10, QChar('0')).arg(seconds,2, 10,QChar('0')));
    feedBackUi->getfeedbackMessage()->setPlainText("You failed to rescue Alex from the undead, Alex DIED "+ Global::instance().getStringValue());
}



void mediumlevel::resetButton(QPushButton *btn)
{
    btn->setEnabled(true);
...
void mediumlevel::progress(int currentQuestionNum, QPushButton *currQuestionBtn){
...
*/
void mediumlevel::optionButtonClicked(QPushButton *btn){
...
void mediumlevel::displayTime(){
    if(td > 0){
        ...
        if (td <= 10) {
            getTimerLabel()->setStyleSheet("color: red;");
        } else {
            getTimerLabel()->setStyleSheet(""); // Reset to default style
        }
...
QTextEdit *mediumlevel::getQuestTextEdit(){
    return ui->textEdit;
}

QPushButton *mediumlevel::getNextBtn(){
    return ui->nextBtn;
}

QPushButton *mediumlevel::getOptionBtn1(){
    return ui->optionBtn1;
}

QPushButton *mediumlevel::getOptionBtn2(){
    return ui->optionBtn2;
}

QPushButton *mediumlevel::getOptionBtn3(){
    return ui->optionBtn3;
}

QPushButton *mediumlevel::getOptionBtn4(){
    return ui->optionBtn4;
}

QPushButton *mediumlevel::getQuestionBtn1(){
    return ui->btn1;
}

QPushButton *mediumlevel::getQuestionBtn2(){
    return ui->btn2;
}

QPushButton *mediumlevel::getQuestionBtn3(){
    return ui->btn3;
}

QPushButton *mediumlevel::getQuestionBtn4(){
    return ui->btn4;
}

QPushButton *mediumlevel::getQuestionBtn5(){
    return ui->btn5;
}

QPushButton *mediumlevel::getQuestionBtn6(){
    return ui->btn6;
}

QPushButton *mediumlevel::getQuestionBtn7(){
    return ui->btn7;
}

QPushButton *mediumlevel::getQuestionBtn8(){
    return ui->btn8;
}

QLabel *mediumlevel::getTimerLabel(){
    return ui->timerLabel;
}


void mediumlevel::on_pevBtn_clicked()
{
...
void mediumlevel::resizeEvent(QResizeEvent *event)
{
